...
// essa função estima a duração da chamada com base no número de mensagens do agente colocando uma média de 30 segundos por mensagem.
function estimateCallDuration(transcript) {
  const AVERAGE_SECONDS_PER_MESSAGE = 30;
  return transcript.length * AVERAGE_SECONDS_PER_MESSAGE;
}

// Função para criar um registro de chamada simulado
function createCallRecord(transcript, complianceResults) {
  return {
    callId: `CALL-${Date.now()}`,
    agentId: 'AGENT-001',
    startTime: new Date().toISOString(),
    durationSeconds: estimateCallDuration(transcript),
    compliant:
      complianceResults.greeting.passed &&
      complianceResults.identification.passed &&
      complianceResults.disclosure.passed,
    customerSentiment: Math.floor(Math.random() * 3) + 3 // mock 3–5
  };
}

// Salvando o registro da chamada no arquivo JSON
...
// conectando todas as partes para processar o transcript e salvar o registro da chamada
try {
  ...
  const callRecord = createCallRecord(transcript, complianceResults);
  saveCallRecord(callRecord);

  console.log(chalk.blue('\nCall record successfully saved to calls.json'));
} catch (error) {
